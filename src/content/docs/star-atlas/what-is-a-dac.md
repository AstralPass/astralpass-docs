---
title: What Is a DAC?
description: A plain-language explanation of what a DAC is in Star Atlas and why DACs matter.
---

In Star Atlas, a **DAC** is a **Decentralized Autonomous Corporation**.

A DAC is basically Star Atlas’ version of a guild, with some DAO elements mixed in.

But, if you are used to games (such as MMOs - Massively Multiplayer Online games) with guilds, clans, or alliances, then a DAC is bascially the Star Atlas equivalent.

The main difference is that Star Atlas imagines these groups as more decentralized, more autonomous, and therefore, hopefully more durable than a normal MMO guild.

## Why DACs exist in Star Atlas

Star Atlas is built around large-scale cooperation.
A single player can only do so much alone.
If players want to build something bigger, hold territory, run fleets, shape trade, or coordinate resources, they need a larger organization.

That is the role of a DAC.

A DAC lets players function as one larger entity instead of acting only as individuals.

## Why the word “decentralized” matters

The “decentralized” part means the organization is not meant to depend entirely on one person controlling everything behind the scenes.

In the Star Atlas vision, a DAC is supposed to be more like an institution than a private friend group. It can have:

- its own identity
- its own rules
- its own shared assets
- more durable internal structure

That does not mean every DAC has to be ultra-serious.
It just means Star Atlas is designed to support organizations that are more structured than a typical guild.

## DAC vs DAO

These terms sound similar, but they are not the same thing.

- **DAC** = a player organization inside Star Atlas
- **DAO** = the broader governance layer for the Star Atlas ecosystem

So if you join a DAC, you are joining a player-run organization.
If you vote in the DAO, you are participating in ecosystem-level governance.

## Why this matters in AstralPass

AstralPass often sits at the point where these worlds meet:

- Discord community access
- wallet ownership
- Player Profiles
- DAC membership on-chain

That is why DACs show up throughout the Star Atlas section of these docs.

If you want to understand how DAC membership affects onboarding, continue with [Joining a DAC On-Chain](/star-atlas/joining-a-dac-on-chain/).
